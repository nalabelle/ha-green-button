...
class GreenButtonXmlStorage:
    ...
    async def async_add_xml(self, xml_data: str, label: str) -> None:
        ...
        existing_index = next(
            (i for i, entry in enumerate(stored_xmls) if entry.get("label") == label), None
        )
...
async def async_migrate_temp_storage(hass: HomeAssistant, unique_id: str, entry_id: str) -> bool:
    ...
    _LOGGER.info(
        "[STORAGE MIGRATION] Successfully migrated %d XML label(s) from temporary storage to .storage/green_button_xml_%s",
        len(temp_xmls),
        entry_id,
    )
