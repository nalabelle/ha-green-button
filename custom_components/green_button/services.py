...
async def async_setup_services(hass: HomeAssistant) -> None:  # noqa: C901
            ...
            for idx, xml_entry in enumerate(stored_xmls):
                ...
                for xml_idx, xml_data in enumerate(xml_list):
                    ...
                    try:
                        ...
                        usage_points = await hass.async_add_executor_job(espi.parse_xml, xml_data)
    ...
    async def import_espi_xml_service(call: ServiceCall) -> None:  # noqa: C901
        ...
        if xml_path:
            ...
            if not file_exists:
                ...
                _LOGGER.error("Checked paths - Original: %s, Resolved: %s", xml_path, resolved_path)
    ...
    async def recalculate_cost_statistics_service(call: ServiceCall) -> None:  # noqa: C901
        ...
        for entry in entries:
            ...
            usage_points = []
            _LOGGER.info(
                "Loading %d stored label(s) from XML storage for recalculation", len(stored_xmls)
            )
            ...
            for idx, xml_entry in enumerate(stored_xmls):
                ...
                for xml_idx, xml_data in enumerate(xml_list):
                    ...
                    xml_count += 1
                    _LOGGER.debug(
                        "Parsing stored XML '%s' [%d/%d]", label, xml_idx + 1, len(xml_list)
                    )
                        ...
                        if not usage_points:
                            ...
                            for new_up in parsed_usage_points:
                                ...
                                if existing_idx is not None:
                                    ...
                                    combined_mrs = list(existing_up.meter_readings) + list(
                                        new_up.meter_readings
                                    )
                                    combined_summaries = list(existing_up.usage_summaries) + list(
                                        new_up.usage_summaries
                                    )
            ...
            _LOGGER.info(
                "Loaded %d usage point(s) from %d XML(s) for recalculation",
                len(usage_points),
                xml_count,
            )
            ...
            if not usage_points:
                _LOGGER.warning(
                    "No usage points found after parsing XMLs for entry %s", entry.title
                )
                continue
            ...
            gas_count = sum(
                1 for up in usage_points if up.sensor_device_class == SensorDeviceClass.GAS
            )
            elec_count = sum(
                1 for up in usage_points if up.sensor_device_class != SensorDeviceClass.GAS
            )
            ...
            for usage_point in usage_points:
                ...
                if is_gas:
                    ...
                    gas_multiplier = (
                        int(raw_gas_multiplier) if raw_gas_multiplier is not None else -5
                    )

                    _LOGGER.info(
                        "Gas cost multiplier for %s: raw=%s, final=%s",
                        entity_id,
                        raw_gas_multiplier,
                        gas_multiplier,
                    )
                    ...
                    mock_entity = MockGasCostEntity(
                        entity_id, entity_state.name or "Gas Cost", "CAD"
                    )
                    ...
                    for idx, mr in enumerate(usage_point.meter_readings, 1):
                        ...
                        total_intervals = (
                            sum(len(blk.interval_readings) for blk in mr.interval_blocks)
                            if mr.interval_blocks
                            else 0
                        )
                        has_cost_data = (
                    ...
                    raw_multiplier = (
                        ...
                        if entry.options.get(CONF_ELECTRICITY_COST_POWER_OF_TEN_MULTIPLIER)
                        is not None
                    ...
                    _LOGGER.info(
                        "Electricity cost multiplier: raw=%s, final=%s", raw_multiplier, multiplier
                    )
                    _LOGGER.info(
                        "Processing %d meter reading(s) for entity %s",
                    ...
                    mock_entity = MockElectricityCostEntity(
                        entity_id, entity_state.name or "Electricity Cost", "CAD"
                    )
                    ...
                    try:
                        ...
                        for idx, meter_reading in enumerate(eligible_electric_mrs, 1):
                            ...
                            _LOGGER.info(
                                "Processing meter reading %d of %d for %s (merge_with_existing=%s)",
                                idx,
                                len(eligible_electric_mrs),
                                entity_id,
                                not is_first,
                        ...
                        _LOGGER.info(
                            "✅ Recalculated electricity cost statistics for %s (%d meter readings processed)",
                            entity_id,
                            len(eligible_electric_mrs),
                        )
                        recalculated_count += 1
