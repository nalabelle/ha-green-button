...
class GreenButtonSensor(CoordinatorEntity[GreenButtonCoordinator], RestoreEntity, SensorEntity):
    ...
    async def async_added_to_hass(self) -> None:  # noqa: C901
        ...
        if self.entity_id and self.native_unit_of_measurement:
            try:
                async_update_statistics_metadata(
                    self.hass,
                    statistic_id=self.entity_id,
                    new_unit_of_measurement=self.native_unit_of_measurement,
                )
            except Exception:  # noqa: BLE001
                _LOGGER.debug(
                    "Sensor %s: Could not update statistics metadata unit (may not exist yet)",
                    self.entity_id,
                )
